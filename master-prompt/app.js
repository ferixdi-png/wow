...
async function loadPrompt(){
 const files=[
  'chunk-01.txt','chunk-02.txt','chunk-03.txt','chunk-04.txt','chunk-05.txt',
  'chunk-06a.txt','chunk-06b.txt','chunk-07a.txt','chunk-07b.txt',
  'chunk-08a.txt','chunk-08b.txt','chunk-09a.txt','chunk-09b.txt',
  'chunk-10a.txt','chunk-10b.txt','chunk-11a.txt','chunk-11b.txt',
  'chunk-12a.txt','chunk-12b.txt'
 ];
 const responses=await Promise.all(files.map(file=>fetch(file,{cache:'no-store'})));
 if(responses.some(response=>!response.ok)) throw new Error('Не удалось загрузить мастер-промпт');
 MASTER_PROMPT=(await Promise.all(responses.map(response=>response.text()))).join('');
 const starts=SECTION_META.map(m=>MASTER_PROMPT.indexOf(m.marker));
 if(starts.some(index=>index<0)) throw new Error('Структура мастер-промпта повреждена');
